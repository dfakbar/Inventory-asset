...
class UpdateSopDocumentRequest extends FormRequest
{
    ...
    {
        return auth()->check() && auth()->user()->can('document.edit');
    }

    private function documentType(): ?SopDocumentType
    {
        $doc = $this->route('document');

        if ($doc instanceof SopDocument) {
            return $doc->document_type;
        }

        return SopDocumentType::tryFrom($this->input('document_type'));
    }

    public function rules(): array
    {
        // Jenis dokumen dikunci — diambil dari dokumen yang sedang diedit,
        // bukan dari input user (mencegah tampering + nomor tetap stabil).
    ...
    {
        if ($this->documentType() !== SopDocumentType::TandaTerima) {
            return;
        }
